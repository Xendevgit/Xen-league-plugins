...
@Slf4j
@Singleton
class LXenPlankMakerOverlay extends OverlayPanel {

	private final Client client;
	private final LXenPlankMakerPlugin plugin;
	private final LXenPlankMakerConfig config;

	String timeFormat;
	private String infoStatus = "Starting...";

	@Inject
	private LXenPlankMakerOverlay(final Client client, final LXenPlankMakerPlugin plugin, final LXenPlankMakerConfig config) {
		super(plugin);
		setPosition(OverlayPosition.BOTTOM_LEFT);
		this.client = client;
		this.plugin = plugin;
		this.config = config;
		getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, "LXenPlankMaker overlay"));
	}
