...
@Slf4j
@Singleton
class LXenSanfewOverlay extends OverlayPanel {

	private final Client client;
	private final LXenSanfewPlugin plugin;
	private final LXenSanfewConfig config;

	String timeFormat;
	private String infoStatus = "Starting...";

	@Inject
	private LXenSanfewOverlay(final Client client, final LXenSanfewPlugin plugin, final LXenSanfewConfig config) {
		super(plugin);
		setPosition(OverlayPosition.BOTTOM_LEFT);
		this.client = client;
		this.plugin = plugin;
		this.config = config;
		getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, "LXenSanfew overlay"));
	}
