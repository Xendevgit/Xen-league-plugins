...
@Slf4j
@Singleton
class LXenZMIOverlay extends OverlayPanel {

	private final Client client;
	private final LXenZMIPlugin plugin;
	private final LXenZMIConfig config;

	String timeFormat;
	private String infoStatus = "Starting...";

	@Inject
	private LXenZMIOverlay(final Client client, final LXenZMIPlugin plugin, final LXenZMIConfig config) {
		super(plugin);
		setPosition(OverlayPosition.BOTTOM_LEFT);
		this.client = client;
		this.plugin = plugin;
		this.config = config;
		getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, "LXenZMI overlay"));
	}
