...
@Slf4j
@Singleton
class LXenBarrowsOverlay extends OverlayPanel {

	private final Client client;
	private final LXenBarrowsPlugin plugin;
	private final LXenBarrowsConfig config;

	String timeFormat;
	private String infoStatus = "Starting...";

	@Inject
	private LXenBarrowsOverlay(final Client client, final LXenBarrowsPlugin plugin, final LXenBarrowsConfig config) {
		super(plugin);
		setPosition(OverlayPosition.BOTTOM_LEFT);
		this.client = client;
		this.plugin = plugin;
		this.config = config;
		getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, "LXenBarrows overlay"));
	}
