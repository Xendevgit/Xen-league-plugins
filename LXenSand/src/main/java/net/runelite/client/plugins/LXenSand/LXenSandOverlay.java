...
@Slf4j
@Singleton
class LXenSandOverlay extends OverlayPanel {

	private final Client client;
	private final LXenSandPlugin plugin;
	private final LXenSandConfig config;

	String timeFormat;
	private String infoStatus = "Starting...";

	@Inject
	private LXenSandOverlay(final Client client, final LXenSandPlugin plugin, final LXenSandConfig config) {
		super(plugin);
		setPosition(OverlayPosition.BOTTOM_LEFT);
		this.client = client;
		this.plugin = plugin;
		this.config = config;
		getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, "LXenSand overlay"));
	}
